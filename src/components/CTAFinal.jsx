...
export default function CTAFinal() {
  ...
  const waLink = `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(t('cf.msg'))}`
    ...
    <section id="cta" className="py-24 md:py-32 relative overflow-hidden">
      ...
      <div className="relative max-w-4xl mx-auto px-6 text-center space-y-10">
        <div className="flex items-center justify-center gap-4">
          ...
          <span className="text-gold text-xs uppercase tracking-[0.3em] font-semibold">{t('cf.tag')}</span>
          <div className="h-px w-12 bg-gold" />
        </div>
        ...
        <div className="space-y-6">
          <h2 className="font-serif text-4xl md:text-5xl lg:text-6xl font-bold text-white leading-tight">
            {t('cf.title.part1')}
            <br />
            {t('cf.title.part2')} <span className="gold-shimmer">{t('cf.title.gold')}</span>
          </h2>
